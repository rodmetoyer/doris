...
for i=1:1:numSweeps
    for j=1:1:numReldensCols
        plot(hax(i),skew(:,j,i),meandepth(:,j,i),char(strcat("-",mrkrs(j),mkrclrs(j))),'MarkerFaceColor',mkrclrs(j),'MarkerSize',6,'DisplayName',num2str(relativeDensity(1,j,i)));
        h(j) = plot(hax(numSweeps+i),skew(:,j,i),meandrift(:,j,i),char(strcat("-",mrkrs(j),mkrclrs(j))),'MarkerFaceColor',mkrclrs(j),'MarkerSize',6,'DisplayName',num2str(relativeDensity(1,j,i)));
        indx = indx +1;
    end
...
hleg = legend(h,'Location','Best','Color',color);
hleg.Title.String = 'Relative Density';
export_fig(hfig,[imagedir 'depthDriftSkewArray' imgNameAppnd '.png'],'-png','-transparent','-m3');
...
for i=1:1:numReldensCols
    plot(ax1,axialLocCm(:,i,1),dPitchdFlowspeed(:,i),char(strcat(standout,mrkrs(i))),...
...
hleg.Title.String = 'Relative Density';
export_fig(hfig1,[imagedir 'dPitchdFlowspeedvAxialLocCM' imgNameAppnd '.png'],'-png','-transparent','-m3');

hfig2 = figure('Position',[baseloc+3*moveright 100 600 400],'Color',color);
...
hfig3 = figure('Position',[baseloc+3*moveright 100 600 400],'Color',color);
...
hfig5 = figure('Position',[baseloc+3*moveright 100 600 400],'Color',color);
...
hfig6 = figure('Position',[baseloc+3*moveright 100 600 400],'Color',color);
