...
for i=1:1:numCases/reldenscols
    ...
    for j=(i-1)*reldenscols+1:1:i*reldenscols
        plot(ax(i),flowfactor(j,:),pitch(j,:),'-o','DisplayName',num2str(dp.relativeDensity(j)));
    end
    hold(ax(i),'off');
    ax(i).XLim = [0.6 0.8];
...
hold(ax4,'on');
for i=1:1:numCases/reldenscols
    plot(ax4,dp.relativeDensity(1:6),dPitchdFF((i-1)*reldenscols+1:i*reldenscols),'-o','DisplayName',num2str(dp.cmaxial((i-1)*reldenscols+1),'%3.2f'));
end
hold(ax4,'off');
ax4.XDir = 'reverse';
