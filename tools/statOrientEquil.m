...
imagedir = '\products\analysis\ESS\';
color = [0.80 0.80 0.80];

sim = simulation.loadsim('ESScase66fromUnder');
a1 = sim.vhcl.centermass(1)/sim.vhcl.body.length;
a3 = sim.vhcl.centermass(3)/sim.vhcl.body.length;
sgma = 0.95;
theta = 0:0.01:2*pi;
...
grid(ax,'on');
ax.Color = 'none';
export_fig(hfig,[pwd imagedir '\' sim.name '.png'],'-png','-transparent','-m3');
...
% export_fig(hfig2,[pwd imagedir '\' sim.name '.png'],'-png','-transparent','-m3');
