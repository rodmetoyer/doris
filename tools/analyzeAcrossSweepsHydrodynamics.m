...
for i=1:1:numCases
    flowfactor(i,:) = [dp.wff(0*length(casesPerSweep) + i) dp.wff(1*length(casesPerSweep) + i) dp.wff(2*length(casesPerSweep) + i) dp.wff(3*length(casesPerSweep) + i)];
    pitch(i,:) = [dp.meanpitch(0*length(casesPerSweep) + i) dp.meanpitch(1*length(casesPerSweep) + i) dp.meanpitch(2*length(casesPerSweep) + i) dp.meanpitch(3*length(casesPerSweep) + i)];
    % (4th sweep - 1st sweep)/flowfactor is delta pitch w.r.t. delta flow factor
    dPitchdFF(i) = (dp.meanpitch(3*length(casesPerSweep) + i)-dp.meanpitch(0*length(casesPerSweep) + i))/(dp.wff(3*length(casesPerSweep) + i)-dp.wff(0*length(casesPerSweep) + i));
end
 % make plots
...
for i=1:1:length(sweeps)
    plot(ax1,pitch(:,i).',dPitchdFF,'*','DisplayName',sweeps(i),'LineWidth',2.0);
end
hold(ax1,'off');
%ax1.XDir = 'reverse';
xlabel('Pitch (deg)');
