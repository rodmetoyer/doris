...
% --generalize> airfoil
    properties (SetAccess = private)
        % Properteis that do not change during simulation
    ...
    methods
        ...
        function [netforce, nettorque] = computeNetAeroLoads(hobj,fluid)
            ...
            Ufluid_B = hobj.P_C_O*fluid.velocity; % Think about how to do this for a vector field for future functionality
            % Rotate rotor center mass velocity to rotor frame
            V_Bx_O_B = hobj.P_C_O*hobj.velocity;
        ...
        function [force, torque, U_relSections, LiftSections, DragSections] = computeAeroLoadArrays(hobj,fluid)
            ...
            Ufluid_B = hobj.P_C_O*fluid.velocity; % Think about how to do this for a vector field for future functionality
            % Rotate rotor center mass velocity to rotor frame
            V_P_O = hobj.P_C_O*hobj.velocity;
            ...
            for i=1:1:hobj.numblades
                ...
                for j=1:1:hobj.blades(i).numsects
                    ...
                    U_rel_bs = transpose(bx_C_a)*hobj.bx_C_P(:,:,i)*U_rel_B;
